...
public class ScheduledReportSend {
    ...
    public void sendReport() throws Exception {
        ...
        if (diffDays == rptPeriod) {
            ...
            Document document = new Document(PageSize.A3.rotate(), 60, 35, 200, 60);
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(scriptPath + "/report/report.pdf"));

            BaseColor titleColor = WebColors.getRGBColor("#63a2e0");
            ...
            img.scaleToFit(1240, 660);
            img.setAbsolutePosition(35, 250);


            Image ilensLogo = Image.getInstance(scriptPath + "/report/logo.png");
            ilensLogo.scaleToFit(100, 100);
            ilensLogo.setAbsolutePosition(55, 800);
            ...
            String subject = "iLens Attendance Report - Reg";
            String msgContent = "<strong>Hi, </strong>" + "<strong>Email is generated by system.</strong>" +
                    "<br>Please find attached complete attendance report for the selected dates between <strong>" + new Date().toString() + "</strong> to <strong>" + new Date().toString() + "</strong>, as scheduled in the system."
